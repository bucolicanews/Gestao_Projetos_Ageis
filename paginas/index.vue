<template>
  <div class="space-y-6">
    ...
    <!-- Filtro por projeto -->
    <div v-if="dados" class="w-full sm:max-w-xs">
      <select
        :value="projetoFiltro ?? ''"
        class="w-full px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primaria"
        @change="selecionarProjeto(($event.target as HTMLSelectElement).value || null)"
      >
        ...
        <option
          ...
          :value="p.id"
        >
          {{ p.nome }}{{ p.tarefas_atrasadas ? ` ⚠️ ${p.tarefas_atrasadas} atrasada(s)` : '' }}
        </option>
      </select>
    </div>
